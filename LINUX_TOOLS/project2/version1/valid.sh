...
# 4 - there was a string where there should have been a number in input file
# 5 - there was a negative number in the items header line (defunct; impossible now)
# 6 - the state in the address of the input file contained a non numeric character or trailing white space
...
# 11 - there was not at least 1 item request
...
# 15 - the awk script found no name on the invoice
# 16 - incorrect number of command line arguments
#
####################################
...
if [[ "$file_ex" != "$INSTATE" && "$file_ex" != "$OUT_STATE" ]]; then
    ...
    if [[ "$status" != "0" ]]; then
        ...
        if [[ "$status" == "1" ]]; then
            echo "ERROR: there was a string in the ITEMS header of this invoice"
            exit 4
        #exit when there was a negative number when there should only be positives
        elif [[ "$status" == "2" ]]; then
            echo "ERROR: there was a negative number in the ITEMS header"
            exit 5
        #awk found a non-letter in the state part of the address
        elif [[ "$status" == "3" ]]; then
            echo "ERROR: there was a non-letter character or trailing white" \
                    "space found in the state field of this invoice"
            exit 6
        #there was no customer name in the invoice file
        elif [[ "$status" == "4" ]]; then
            echo "ERROR: there was no customer name for this invoice"
            exit 15
        fi
    fi
